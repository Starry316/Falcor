...
)
{
    ...
    for (int i = 0; i < values_count; i++)
    {
        pos[i] = ImLerp(inner_bb.Min, inner_bb.Max, ImVec2(values_getter(data, 2 * i), values_getter(data, 2 * i + 1)));
    }
    ...
    if (hovered && inner_bb.Contains(g.IO.MousePos) && g.IO.MouseDown[0])
    {
        if (values_getter(data, 2 * values_count) == 1.0f)
        {
            ((float*)data)[2] = (g.IO.MousePos.x - inner_bb.Min.x) / inner_bb.GetWidth();
            ((float*)data)[3] = (g.IO.MousePos.y - inner_bb.Min.y) / inner_bb.GetHeight();
            flag_edit = true;
        }
        else if (values_getter(data, 2 * values_count + 1) == 1.0f)
        {
            ((float*)data)[4] = (g.IO.MousePos.x - inner_bb.Min.x) / inner_bb.GetWidth();
            ((float*)data)[5] = (g.IO.MousePos.y - inner_bb.Min.y) / inner_bb.GetHeight();
            flag_edit = true;
        }
    }
    ...
    if (overlay_text)
        RenderTextClipped(
            ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y), frame_bb.Max, overlay_text, NULL, NULL, ImVec2(0.5f, 0.0f)
        );
...
{
    return PlotBezierCurveEx(label, values_getter, data, values_count, overlay_text, graph_size);
}
