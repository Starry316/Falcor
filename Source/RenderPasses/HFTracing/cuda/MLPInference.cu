...
__global__ void inferInt8(int* weight, int* input, float* output, unsigned int width, unsigned int height, int* validMask)
{
    ...
    if(validMask[y * width + x] == 0) return;
...
__global__ void inferFP16(__half* weighth2, __half* biash2, int* input, float* output, unsigned int width, unsigned int height, int* validMask)
{
...
__global__ void inferFP32(float* weight2, float* bias2, float* input, float* output, unsigned int width, unsigned int height, int* validMask)
{
    __shared__ float weight[2048];
    __shared__ float bias[76];
...
void launchInferFP32(float* weight, float* bias, float* input, float* output, unsigned int width, unsigned int height, int* validMask)
{
    dim3 dimBlock(16, 16);
    dim3 dimGrid((width + dimBlock.x - 1) / dimBlock.x, (height + dimBlock.y - 1) / dimBlock.y);
    inferFP32<<<dimGrid, dimBlock>>>(weight, bias, input, output, width, height, validMask);
}
void launchInferFP16(__half* weighth, __half* biash, int* input, float* output, unsigned int width, unsigned int height, int* validMask)
{
    dim3 dimBlock(16, 16);
    dim3 dimGrid((width + dimBlock.x - 1) / dimBlock.x, (height + dimBlock.y - 1) / dimBlock.y);
    inferFP16<<<dimGrid, dimBlock>>>(weighth, biash, input, output, width, height, validMask);
}
void launchInferInt8(int* weight, int* input, float* output, unsigned int width, unsigned int height, int* validMask)
{
    dim3 dimBlock(16, 16);
    dim3 dimGrid((width + dimBlock.x - 1) / dimBlock.x, (height + dimBlock.y - 1) / dimBlock.y);
    inferInt8<<<dimGrid, dimBlock>>>(weight, input, output, width, height, validMask);
}
