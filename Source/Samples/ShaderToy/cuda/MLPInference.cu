...
)
{
    dim3 dimBlock(16, 16);
    dim3 dimGrid((width + dimBlock.x - 1) / dimBlock.x, (height + dimBlock.y - 1) / dimBlock.y);
    testTexFP32<<<dimGrid, dimBlock>>>(weight, bias, HP, DP, UP, output, width, height);
}

__global__ void testTex(
    int* weight,
    float* testInput,
...
)
{
    dim3 dimBlock(16, 16);
    dim3 dimGrid((width + dimBlock.x - 1) / dimBlock.x, (height + dimBlock.y - 1) / dimBlock.y);
    testTex<<<dimGrid, dimBlock>>>(weight, testInput, HP, DP, UP, output, width, height);
}
